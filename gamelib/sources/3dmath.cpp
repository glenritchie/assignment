...
//		ALMOST ALL THIS CODE IS STRAIGHT FROM THESE LESSONS.
//		MOST OF IT THOUGH IS VERY HEAVILY MODIFIED BY ME!
//*************************************************************
...
#define _isnan isnan
#endif
//inline float sqrt(float r) { return sqrtf(r); }
//inline double sqrt(double r) { return sqrt(r); }
inline double fsqrt(double r) { return sqrt(r); }
inline float fsqrt(float r) { return sqrt(r); }
/*		
//	NOTE: This code is NOT portable! 
//  Compiler options '-O2 -march=pentium2 -funroll-loops' cause this function to return results that equal: 1.#J
		double x,y;
		double tempf;
...
/*
double Magnitude(const VECTOR3 &vNormal)
{
...
*/
/*
double Magnitude(VECTOR2 &vNormal)
{
	// This will give us the magnitude or "Norm" as some say, of our normal.
	// Here is the equation:  magnitude = sqrt(V.x^2 + V.y^2)  Where V is the vector
	return fsqrt( (vNormal.x * vNormal.x) + (vNormal.y * vNormal.y));
}
*/
...
*/
/*
VECTOR3 Normalize(const VECTOR3 &vNormal)
{
...
*/
/*
double Distance(VECTOR3 &vPoint1, VECTOR3 &vPoint2)
{
...
*/
/*
double Distance(VECTOR2 vPoint1, VECTOR2 vPoint2)
{
	// This is the classic formula used in beginning algebra to return the
	...
	//
	double distance = fsqrt( (vPoint2.x - vPoint1.x) * (vPoint2.x - vPoint1.x) + (vPoint2.y - vPoint1.y) * (vPoint2.y - vPoint1.y));
	// Return the distance between the 2 points
	return distance;
}
*/
...
{
	VECTOR3 vNormal;									// The vector to hold the cross product
...
{	
	double distance = 0;									// This variable holds the distance from the plane tot he origin
...
bool IntersectedBox(VECTOR3 &startOfLine, VECTOR3 &endOfLine, VECTOR3 &nmin, VECTOR3 &nmax)
{
	// Get the 6 planes of the box
	VECTOR3 min = nmin;
	VECTOR3 max = nmax;
//	min.x += origin.x;	min.y += origin.y;	min.z += origin.z;
//	max.x += origin.x;	max.y += origin.y;	max.z += origin.z;
	double polys[][3] = {
		// Bottom
		{ min.x, min.y,	min.z },
		{ max.x, min.y,	max.z },
...
//	VECTOR3 norm = Normal(n);
//	findPlane(bottomPlane, bottom[0], bottom[1], bottom[2]);
	VECTOR3 line[2] = {	startOfLine, endOfLine };
	for (int i = 0; i < 18; i+=3)
	{
	 	VECTOR3 p[] = { 
			VECTOR3(polys[i+0][0], polys[i+0][1], polys[i+0][2]),
			VECTOR3(polys[i+1][0], polys[i+1][1], polys[i+1][2]),
			VECTOR3(polys[i+2][0], polys[i+2][1], polys[i+2][2])};
			VECTOR3 norm = Normal(p);
			//bool b = IntersectedPolygon(p, line, 3);
			double dist;
			VECTOR3 n;
			bool b = IntersectedPlane(p, line, n, dist);
			if (b)
				return true;
	}
	return false;
...
/*double Dot(VECTOR3 vVector1, VECTOR3 vVector2) 
{
...
/////////////////////////////////// ANGLE BETWEEN VECTORS \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/** THIS IS STUPID! 
 SEE THE SWEAR WORDS BELOW
*/
void doNothing(int v1, char *str, ...)
{
...
double AngleBetweenVectors(const VECTOR3 &Vector1, const VECTOR3 &Vector2)
{
	...
	if( vectorsMagnitude == 0.0)						// Check so we don't divide by zero
		return 0;															// Return an arbitrary point on the line
	/////////////////////////////////////////////
	// ADDED: By Spikeles - Prevent DOMAIN errors!
	/////////////////////////////////////////////
	double cosme = dotProduct / vectorsMagnitude;
//	DebugText(EMSG, "cosme: %f", cosme);
	/** ARRRGH! THIS IS SO STUPID. THIS FUNCTION CALL NEEDS TO BE HERE ELSE
   * SOMETHING TO DO WITH acos or something screws up! STUPID g++!!!!
	*/
	doNothing(EMSG, "cosme: %f", cosme);
	
	if (cosme > 1.0 || cosme < -1.0)
		return 0;
	/////////////////////////////////////////////

	double angle = acos(cosme);
				   ...
				   VECTOR3 &vNormal, VECTOR3 &vPoint, float radius, float &distance)
{
	...
	double d = PlaneDistance(vNormal, vPoint);
						 ...
						 VECTOR3 vPolygon[], int vertexCount, float radius)
{
	...
	for(int i = 0; i < vertexCount; i++)
	{
		...
		double distance = Distance(vPoint, vCenter);
...
/**
  Changed to return the intersection point and distance!
*/
bool SpherePolygonCollision(VECTOR3 vPolygon[], 
							VECTOR3 &vCenter, int vertexCount, float radius, 
							float &distance, VECTOR3 &vPosition)
{
	// 1) STEP ONE - Finding the sphere's classification
...
/////////////////////////////////////////////
// 	Added by Spikeles
//  variation of IntersectedPolygon(VECTOR3 vPoly[], VECTOR3 vLine[], int verticeCount)
// 	that returns the intersection point
/////////////////////////////////////////////
VECTOR3 PointonTriangle(VECTOR3 vPoly[], VECTOR3 vLine[], int verticeCount, VECTOR3 &vNormal2)
{
	VECTOR3 vNormal = vNormal2;
	VECTOR3 errvec(0,0,0) ;
	double originDistance = 0;
...
//		- "Talk to me like I'm a 3 year old!" Programming Lessons -		 //
//                                                                       //
//		$Author: spikeles $Program:		Bezier Curve									 //
//																		 //
...
// Find if an entity is in the cube
bool isPointInCube(CVector3 &cubeOrigin, const float &radius, const CVector3 &p )
{
	CVector3 cubeMin, cubeMax;
	float originminusx = cubeOrigin.x - radius;
	float originminusy = cubeOrigin.y - radius;
	float originminusz = cubeOrigin.z - radius;
	float originplusx = cubeOrigin.x + radius;
	float originplusy = cubeOrigin.y + radius;
	float originplusz = cubeOrigin.z + radius;
	cubeMin.x = originminusx;
	cubeMin.y = originminusy;
	cubeMin.z = originminusz;
	cubeMax.x = originplusx;
	cubeMax.y = originplusy;
	cubeMax.z = originplusz;

	if	(
			((p.x >= cubeMin.x) && (p.x <= cubeMax.x)) &&
